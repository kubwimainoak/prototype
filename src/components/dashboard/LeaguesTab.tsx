  ...
  const LeagueDetailsModal = ({ league }: { league: League }) => {
    return (
      <Dialog open={openLeagueId === league.id} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto p-2">
          <DialogHeader>
          ...
          <Tabs defaultValue="info" className="mt-4">
            <TabsList className="w-full border border-secondary bg-card rounded-md">
              <TabsTrigger
                value="info"
                className=" text-[12px] sm:text-[14px] data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm"
              >
                Information
              </TabsTrigger>
              <TabsTrigger 
